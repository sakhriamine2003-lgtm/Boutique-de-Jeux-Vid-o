
const container = document.getElementById("cards");

export function afficherGames(list) {
  container.innerHTML = "";

  for (let i = 0; i < list.length; i++) {
    let game = list[i];

    container.innerHTML += `
      <div 
        class="h-64 rounded-[10px] flex flex-col justify-end w-full md:w-[31%] mx-1"
        style="background-image: url('${game.image}'); background-size: cover; background-position: center;"
      >
    <button 
        onclick="AjouterPanier(${game.id})"
        class="ml-[85%] mb-[34%] bg-gradient-to-r from-orange-200 to-orange-400 w-10 rounded-[50px] p-1">
        <img src="https://cdn-icons-png.flaticon.com/128/4947/4947050.png" alt="acheter">
    </button>

        <div class="bg-[#3C3B3B]/50 flex justify-between items-center text-white font-bold p-4">
          <p class="">${game.title}</p>
          <div class="text-right">
            <p>${game.category}</p>
            <p>${game.price} €</p>
          </div>
        </div>
      </div>
    `;
  }
}
